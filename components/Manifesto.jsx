...
/* Single strike line that animates its underline on scroll */
function StrikeLine({ text, delay }) {
  ...
  return (
    <span
      ref={ref}
      className="relative inline-block text-white/22 font-display font-extrabold leading-tight"
      style={{ transitionDelay: `${delay}ms` }}
    >
      {text}
      {/* Strikethrough bar */}
      <motion.span
        className="absolute left-0 top-[55%] h-[3px] bg-forge-amber rounded-full block"
        initial={{ width: '0%' }}
...
export default function Manifesto() {
  return (
    <section
      id="about"
      className="relative z-10 border-t border-forge-border"
    >
      <div className="max-w-6xl mx-auto px-6 py-24 lg:py-32">
        ...
        <Reveal delay={0.1}>
          <h2 className="font-display text-[44px] lg:text-[56px] font-extrabold leading-[1.1] tracking-[-1px] mb-8 flex flex-col gap-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          {/* Left — body copy */}
          <Reveal delay={0.15}>
            <p className="text-[17px] text-white/40 leading-[1.8] font-light">
              Most businesses are drowning in tasks that a well-built AI agent
              could handle in seconds. We build the systems that make that
              happen — no fluff, no slide decks, just working software in your
              stack.
            </p>
          </Reveal>
            ...
            {PRINCIPLES.map((text, i) => (
              <Reveal key={text} delay={0.18 + i * 0.1}>
                <div className="flex items-center gap-4 py-5">
                  ...
                  <p className="font-display text-[14px] font-semibold text-white/80 leading-snug">
                    {text}
                  </p>
                </div>
              </Reveal>
